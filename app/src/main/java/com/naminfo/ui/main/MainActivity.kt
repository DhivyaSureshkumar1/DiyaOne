...
import com.naminfo.databinding.MainActivityBinding
...
import com.naminfo.ui.main.viewmodel.MainViewModel
...
class MainActivity : GenericActivity() {
    ...
    private val assistantLauncher = registerForActivityResult(
        ...
        coreContext.postOnCoreThread { core ->
            if (core.accountList.isNotEmpty()) {
                coreContext.postOnMainThread {
                    ViewModelProvider(this)[com.naminfo.ui.main.contacts.viewmodel.ContactsListViewModel::class.java]
                        .applyCurrentDefaultAccountFilter()
                    findNavController().navigate(
                        R.id.conversationsListFragment, null,
                        NavOptions.Builder().setPopUpTo(R.id.main_nav_graph, false)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.lastAccountRemovedEvent.observe(this) {
            it.consume {
                assistantLauncher.launch(Intent(this, AssistantActivity::class.java))
            }
        }
        ...
        coreContext.postOnCoreThread { core ->
            if (corePreferences.firstLaunch) {
                Log.i("$TAG First launch, skipping welcome and opening one-time registration")
    ...
    private fun goToLatestVisitedFragment() {
        try {
            ...
            // Always open the Messages tab on a normal app launch. Intent handling below can
            // still navigate to a specific call, contact, conversation, or notification target.
            val defaultFragmentId = CHAT_FRAGMENT_ID
            Log.i(
