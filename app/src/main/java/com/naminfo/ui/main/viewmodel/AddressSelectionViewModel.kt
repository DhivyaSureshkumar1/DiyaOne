...
abstract class AddressSelectionViewModel
    ...
    protected open val automaticallySelectPreferredAddress = false

    protected open val showAddressSuggestions = true

    protected open val limitContactSearch = true

    protected open val contactSearchDomain: String
        get() = corePreferences.contactsFilter

    @WorkerThread
    protected open fun shouldIncludeFriend(friend: Friend): Boolean = true

    @WorkerThread
    protected open fun beforeProcessingSearchResults() { }
    ...
    @WorkerThread
    protected open fun additionalContacts(filter: String): List<Friend> = emptyList()
    ...
    init {
        ...
        coreContext.postOnCoreThread { core ->
            ...
            magicSearch.limitedSearch = limitContactSearch
    ...
    private fun applyFilter(
        ...
        val domain = contactSearchDomain
        Log.i(
    ...
    private fun processMagicSearchResults(results: Array<SearchResult>) {
        ...
        for (result in favorites) {
            ...
            if (!shouldIncludeFriend(friend)) continue
        ...
        for (result in results) {
            ...
            } else if (address != null && showAddressSuggestions) {
        ...
        for (friend in additionalContacts(currentFilter)) {
            val address = LinphoneUtils.getFirstAvailableAddressForFriend(friend) ?: continue
            if ((contactsList + favoritesList).any { it.address.weakEqual(address) }) continue
            val model = ConversationContactOrSuggestionModel(address, friend = friend)
            model.avatarModel.postValue(coreContext.contactsManager.getContactAvatarModelForFriend(friend))
            contactsList.add(model)
        }
    ...
    fun handleClickOnContactModel(model: ConversationContactOrSuggestionModel) {
        ...
        coreContext.postOnCoreThread { core ->
            ...
            val preferredAddress = LinphoneUtils.getSingleAvailableAddressForFriend(friend)
                ?: if (automaticallySelectPreferredAddress) {
                    LinphoneUtils.getFirstAvailableAddressForFriend(friend) ?: model.address
                } else {
                    null
                }
            if (preferredAddress != null) {
                Log.i(
                    "$TAG Using preferred address for contact [${friend.name}]"
                )
                onAddressSelected(preferredAddress, friend)
            } else {
