...
import com.naminfo.ui.call.model.CallMediaEncryptionModel
import com.naminfo.ui.call.model.CallStatsModel
import com.naminfo.ui.main.contacts.model.ContactAvatarModel
import com.naminfo.ui.main.history.model.NumpadModel
...
class CurrentCallViewModel
    ...
    val activeCallName = displayedName.map { value ->
        val name = value.orEmpty().substringBefore('<').trim().trim('"')
        if (name.isEmpty() || name.contains('@') ||
            name.startsWith("sip:", ignoreCase = true) ||
            name.startsWith("sips:", ignoreCase = true)
        ) {
            AppUtils.getString(R.string.active_call_unknown_caller)
        } else {
            name
        }
    }
    ...
    val requestRecordAudioPermissionToAnswer: MutableLiveData<Event<Boolean>> by lazy {
        MutableLiveData()
    }
    ...
    fun answerCallFrom(caller: String) {
        coreContext.postOnCoreThread { core ->
            val call = core.calls.find {
                LinphoneUtils.isCallIncoming(it.state) && (caller.isEmpty() || caller == it.remoteAddress.asStringUriOnly())
            }
            if (call != null) {
    ...
    fun answer() {
        if (ActivityCompat.checkSelfPermission(
            ...
            Log.w("$TAG RECORD_AUDIO permission isn't granted, requesting it before answering")
            requestRecordAudioPermissionToAnswer.postValue(Event(true))
            return
        }
    ...
    private fun configureCall(call: Call) {
        ...
        val uri = address.username.orEmpty()
