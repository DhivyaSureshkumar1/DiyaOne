...
import com.naminfo.telecom.RedirectionHandler
...
class CallActivity : GenericActivity() {
    ...
    private val requestRecordAudioPermissionToAnswerLauncher = registerForActivityResult(
        ActivityResultContracts.RequestPermission()
    ) { isGranted ->
        if (isGranted) {
            Log.i("$TAG RECORD_AUDIO permission has been granted, answering incoming call")
            callViewModel.answer()
        } else {
            Log.e("$TAG RECORD_AUDIO permission has been denied, incoming call wasn't answered")
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        callViewModel.goToEndedCallEvent.observe(this) {
            it.consume { message ->
                ...
                // A video call can still be marked as video-enabled after it reaches End.
                // Disable automatic PiP before showing the ended-call screen so finishing
                // this activity doesn't trigger another PiP lifecycle transition.
                if (isPipSupported) {
                    Compatibility.enableAutoEnterPiP(this, false)
                }
        ...
        callViewModel.requestRecordAudioPermissionToAnswer.observe(this) {
            it.consume {
                Log.w("$TAG Asking for RECORD_AUDIO permission before answering incoming call")
                requestRecordAudioPermissionToAnswerLauncher.launch(Manifest.permission.RECORD_AUDIO)
            }
        }
    ...
    fun returnToMainActivityAfterCall() {
        Log.i("$TAG Call ended, returning to MainActivity")
