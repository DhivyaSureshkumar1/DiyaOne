...
import com.naminfo.utils.ImageUtils
...
class ContactsManager
    ...
    private val magicSearchListener = object : MagicSearchListenerStub() {
        ...
        override fun onSearchResultsReceived(magicSearch: MagicSearch) {
            ...
            if (results.isNotEmpty()) {
                val result = results.firstOrNull { it.friend != null }
                if (result != null) {
        ...
        override fun onPresenceReceived(friendList: FriendList, friends: Array<out Friend?>) {
            if (friendList.isSubscriptionBodyless) {
                ...
                reloadPresenceContactsJob = coroutineScope.launch {
                    ...
                    coreContext.postOnCoreThread { notifyContactsListChanged() }
                }
            }
        }
    ...
    private val coreListener: CoreListenerStub = object : CoreListenerStub() {
        ...
        override fun onAccountRegistrationStateChanged(
            core: Core, account: Account, state: org.linphone.core.RegistrationState?, message: String
        ) {
            if (state == org.linphone.core.RegistrationState.Ok && account == core.defaultAccount) {
                refreshPresenceAfterLogin(core)
            }
        }
        ...
        override fun onDefaultAccountChanged(core: Core, account: Account?) {
            ...
            if (account?.state == org.linphone.core.RegistrationState.Ok) {
                refreshPresenceAfterLogin(core)
            }
        }
    }
    ...
    @WorkerThread
    private fun refreshPresenceAfterLogin(core: Core) {
        // The foreground event may happen before the account is registered.
        if (corePreferences.publishPresence && !core.isInBackground) {
            core.consolidatedPresence = org.linphone.core.ConsolidatedPresence.Online
        }
        core.isFriendListSubscriptionEnabled = true
        for (list in core.friendsLists) {
            if (list.isSubscriptionsEnabled) list.updateSubscriptions()
        }
        notifyContactsListChanged()
    }
